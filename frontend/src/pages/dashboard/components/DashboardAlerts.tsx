...
import { Card, List, Tag, Alert, Spin, Typography, Input, Button, Space, Row, Col } from 'antd';
import { 
    ClockCircleOutlined, 
...
export default function DashboardAlerts() {
    ...
    useEffect(() => {
        request.get('/standards/dashboard-alerts/')
            .then(res => {
                ...
                <Card 
                    title={
                        <Space>
                            ...
                            <span style={{ fontSize: 16, fontWeight: 600, color: '#1e293b' }}>实时更新专区 (30天内即将实施)</span>
                        </Space>
                    } 
                    style={{ height: '100%', borderRadius: 16, boxShadow: '0 4px 20px -8px rgba(0,0,0,0.05)', border: 'none' }}
                    styles={{
                    ...
                    extra={
                        <Space>
                            ...
                            <Button 
                                ...
                                title={sortOrder === 'asc' ? "当前: 紧急优先 (点击切换)" : "当前: 宽松优先 (点击切换)"}
                            />
                        </Space>
                    }
                >
                    <Spin spinning={loading}>
                        <List
                            ...
                            renderItem={item => (
                                ...
                                >
                                    <List.Item.Meta
                                        title={<Text strong style={{ fontSize: 15, color: '#334155' }}>{item.bz_id} {item.bz_name}</Text>}
                                        description={<span style={{ color: '#94a3b8' }}>实施日期: {item.implement_time}</span>}
                                    />
                                    <div>
                                        <Tag color={getDaysTagColor(item.days_left)} style={{ fontSize: '14px', padding: '4px 10px', borderRadius: 6, marginRight: 0 }}>
                                            仅剩 {item.days_left} 天
                                        </Tag>
                                    </div>
                                </List.Item>
                            )}
                            locale={{ emptyText: <div style={{ padding: '40px', color: '#94a3b8' }}>近期暂无即将实施的标准</div> }}
                        />
                    </Spin>
                </Card>
            </Col>
                ...
                >
                    <Spin spinning={loading}>
                        <List
                            ...
                            renderItem={item => (
                                <List.Item style={{ borderBottom: 'none', padding: '0 0 12px 0' }}> 
                                    <Alert
                                        message="标准废止提醒"
                            ...
                            locale={{ emptyText: 
                                <div style={{ padding: '24px', background: '#eff6ff', borderRadius: 10, border: '1px solid #bfdbfe', display: 'flex', alignItems: 'center', gap: 10, color: '#1d4ed8' }}>
                                    <NotificationOutlined style={{ fontSize: 16, color: '#3b82f6' }} />
