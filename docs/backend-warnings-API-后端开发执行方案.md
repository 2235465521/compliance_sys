...
flowchart TB
  subgraph http [HTTP 前缀 /api]
    ...
    G["GET /warnings/monitor/enterprises/detail/?qb_code="]
...
| 输出字段 | 建议数据来源 |
|----------|----------------|
| `referenced_std_code` | 最近一次合规 Step3 映射表 / 批量 `references_resolved[].referenced_std_code`；按规范化去重 |
| `full_std_at_publication` | 同上记录的 `full_std_at_publication`；多条历史取**最近一条**非空 |
| `baseline_latest_std` | 表 **`novelty_reference_baseline_snapshot.baseline_latest_std_primary`**，键 `(qb_code, referenced_std_code)`，**只读** |
| `current_latest_std` | 实时谱系查询，与查新任务 `current_latest_std_primary`、合规 `reference-latest` 同一引擎 |
...
| `all_ok_count` | int | 状态良好企标数 |
| `pending_count` | int | 可选，处理中数量 |
...
| `status` | string | — | `need_attention` \| `all_ok` \| `all` |
...
### 3.5 接口 G — 单企标监控明细（下钻 / Drawer，推荐）

| 项目 | 值 |
|------|-----|
| **路径** | `GET /api/warnings/monitor/enterprises/detail/` |
| **Query** | `qb_code`（必填） |
| **说明** | 企标号常含 `/`（如 `Q/15763.2-2005`），**必须**用 query 传参；路径参数 `.../enterprises/{qb_code}/` 在多数环境下会把 `%2F` 解析为路径分隔符导致 404 |

#### 与列表接口区分

| 接口 | 用途 |
|------|------|
| `GET .../monitor/enterprises/` | 分页列表（监控 Tab 表格） |
| `GET .../monitor/enterprises/detail/?qb_code=...` | 单个企标五列明细（点击某行后） |

#### cURL

```bash
curl -G "http://127.0.0.1:8000/api/warnings/monitor/enterprises/detail/" \
  --data-urlencode "qb_code=Q/15763.2-2005" \
  -H "Authorization: Bearer YOUR_TOKEN"
```
...
仅当企标号**不含** `/` 时可使用 `GET /api/warnings/monitor/enterprises/{qb_code}/`。前端已统一使用 query 接口。
...
| `message` | string | 提示文案 |
| `job_id` | string | 异步任务 ID（可选） |
| `processed_count` | int | 同步完成时处理的企标数 |
...
sequenceDiagram
  ...
  loop 每个已评价企标
    Job->>SVC: qb_code
    SVC-->>Job: compare_rows
    Job->>DB: 写入 snapshot
  end
  Job->>DB: 更新 monitor_run 汇总
  FE->>FE: GET summary / enterprises
```
...
| G | GET | `/warnings/monitor/enterprises/detail/?qb_code=` | 监控下钻明细 |
