...
| 顺序 | 方法 | 路径 | 说明 |
|------|------|------|------|
| 4-1 | GET | `/api/v1/compliance/evaluations/{task_id}/step/3/reference-latest` | **必须**已 **3-2** 成功且服务端 **`current_step >= 3`**；返回数组，每条为引用解析结果（现行号、谱系链、`historical_full_std_code` 等） |
...
**SQLite / 无映射数据**：`reference-latest` 可能为 **200 + `[]`**，需结合是否 MySQL、映射表是否有数据判断，不是接口未调用成功。

### 阶段 5：步骤 4 — 指标编排与审核 4

| 顺序 | 方法 | 路径 | 说明 |
|------|------|------|------|
| 5-1 | **POST** | `/api/v1/compliance/evaluations/{task_id}/step/4/indicators/ensure` | **第五步主路径**：body 传 ③ `compare_pairs`；返回 `all_ready`、`missing_gb_files`、指标表。详见 [step5-indicator-ensure 变更说明](./backend-compliance-step5-indicator-ensure-API变更说明.md) |
| 5-1b | GET | `/api/v1/compliance/evaluations/{task_id}/step/4/indicators` | 按映射表 n+m 编排（保留）；第五步 UI **不再**依赖此 GET |
| 5-2 | （按需）POST | `/api/v1/compliance/national-standards/upload?std_code=...` | 对 `missing_gb_files` 补传国标；补完后**再 POST 5-1** |
...
| reference-latest **200** 且 **`[]`** | 多为 SQLite、或 MySQL 下映射表无有效 `referenced_std_code` |
