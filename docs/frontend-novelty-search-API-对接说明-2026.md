...
### 6.5 `PATCH /tasks/{task_id}/reference-sheet` — 保存专用表草稿
...
### 6.6 `POST /tasks/{task_id}/confirm-sheet` — 确认并比对
...
### 6.7 `POST /tasks/{task_id}/retry` — 重试
...
### 6.8 `GET /tasks/{task_id}/indicators` — 指标明细
...
### 6.9 `POST /tasks/{task_id}/report` — PDF 报告
...
| 任务列表页 | `GET /tasks` + 筛选 `qb_code` / `status` / `keyword` |
