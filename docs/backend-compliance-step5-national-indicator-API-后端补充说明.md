...
|--------|------|------|------|-------------------|
| **P0** | `PUT` | `/api/v1/compliance/evaluations/{task_id}/step/4/national-indicators` | 保存用户编辑后的指标 JSON | **待新增** |
| P1 | `GET` | 同上路径 + `?std_code=` | 弹窗加载指标明细 | 建议已实现，联调确认 |
| P1 | `POST` | `.../national-indicators/review` | 人工「审核通过」 | 建议已实现，联调确认 |
| P2 | — | `POST .../step/4/indicators/ensure` 响应增强 | `national_by_std_code` 带出 `manual_review_status` | 建议已实现，联调确认 |
...
- [ ] **P0** 新增 `PUT .../step/4/national-indicators`（`NationalIndicatorSaveIn` / `NationalIndicatorSaveOut`）
- [ ] **P0** PUT：校验 JSON 数组格式；404/422 与上文一致
- [ ] **P1** 确认 `GET .../national-indicators` 返回 `manual_review_status`
- [ ] **P1** 确认 `POST .../national-indicators/review` 可用
- [ ] **P2** 确认 `POST .../indicators/ensure` 的 `national_by_std_code` 含 `manual_review_status`
- [ ] 单测建议：pending → PUT 改内容（仍为 pending）→ review approved → GET 内容一致
