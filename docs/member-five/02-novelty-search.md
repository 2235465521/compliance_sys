...
本文档依据「查新任务」业务流程图（任务发起、解析与专用表初稿、人工确认、逐条查库与溯源、三态结论、汇总展示、PDF 报告与归档）与《标准化信息服务平台——需求分析文档》第 8.2 节整理。**主流程 Mermaid 图与补充说明见第 2.1～2.3 节。** 不实现合规模块界面；流程图中「查询后端标准库 / 模块1」为后端行为，前端仅在比对明细中展示结果。**HTTP 契约以 [backend-novelty-search-API-后端对接手册.md](../backend-novelty-search-API-后端对接手册.md) 为准。**

### 三列查新（2026-05 实现要点）

| 列 | 含义 |
|----|------|
| 补全年代号的标准号 | 企标发布时点完整国标（`full_std_at_publication`） |
| 首次查新时最新标准号 | 该企标在系统中**最早一次**合规/批量查新完成时的 `latest_std_primary` 快照 |
| 本次查新最新标准号 | 当前任务实时谱系查询结果 |

任务详情 **「比对明细（三列）」** Tab 展示上述三列 + 行级结论；**「历史指标」** Tab 展示汇聚的企标/国标指标。

前端仅调用 `/api/v1/novelty-search` 后端接口，不再提供本地 Mock 数据模式。
...
2. **比对明细（三列）**：补全年代号 / 首次查新最新号 / 本次最新号 + 行级结论。
3. **历史指标**：历次合规/批量评价汇聚的企标与国标指标。
4. **报告与历史**：生成 PDF、预览、下载；本任务时间线（可选）。
...
- **比对明细 Tab**：三列查新表 + 任务级结论 Alert；比对进行中显示进度。
- **历史指标 Tab**：`GET .../tasks/{id}/indicators` 或详情内嵌载荷。
