...
**写入时机**（仅当 `(subject_code, referenced_std_code)` **不存在** 快照时插入）：
...
对请求中的 `subject_code` 做规范化（trim、统一连字符、可选忽略大小写）后匹配：

| 数据源 | 匹配字段 |
|--------|----------|
| `compliance_evaluation_task` | `subject_code`（Step1 确认后） |
| `batch_normative_reference_item` | `subject_code` 及解析结果中的同义字段 |
...
`POST /tasks` 时若 `subject_code` 在系统中 **无任何** 已完成 Step3 的合规任务或已完成批量子项：
...
|------|------|------|
| `id` | PK | |
| `title` | string | 如 `{subject_code} {date}` |
| `subject_code` | string | 企标号 |
| `status` | string | §2.5 |
...
| POST | `/tasks` | 创建任务（`subject_code` 必填；可选 `file`） |
...
| `subject_code` | string | 模糊或精确匹配企标号 |
...
### 6.8 `GET /tasks/{task_id}/indicators`

**响应 200**：`NoveltyIndicatorsOut`（§7.4）

---

### 6.9 `POST /tasks/{task_id}/report`（可选）
