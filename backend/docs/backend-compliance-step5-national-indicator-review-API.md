...
| `NationalIndicatorReviewModal.tsx` | 指标明细 + 审核通过/驳回 |
| `ComplianceWizardPanel.tsx` | 上传后 `parsingStdCodes` + 轮询 ensure |
| `compliance-api.ts` | `getNationalIndicatorsByStd`、`postNationalIndicatorReview` |

GET/POST 未实现时：指标弹窗回退 ensure 缓存的 `national_by_std_code`；审核接口报错提示后端未就绪。

---

## 7. 后端实现清单

- [x] `_fetch_national_indicator_rows` 增加 `manual_review_status`
- [x] `GET .../step/4/national-indicators?std_code=`
- [x] `POST .../step/4/national-indicators/review`
- [ ] （可选）`all_ready` 纳入 `approved` 判定
- [x] 单测：上传 → ensure → GET 为 pending → review approved → ensure 仍为 ready
...
| 2026-05-24 | 后端实现 GET/POST national-indicators；ensure 响应带出 manual_review_status |
