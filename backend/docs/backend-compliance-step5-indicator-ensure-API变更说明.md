...
- [x] `ComparePairIn`  
- [x] `Step4IndicatorsEnsureIn`  
- [x] `StdCodeOrchestrationStatus`  
- [x] `Step4IndicatorsEnsureOut`（`all_ready` + `std_statuses` + 继承现有字段）

### 6.2 服务层（`backend/apps/compliance/services/evaluation_flow.py`）

- [x] 抽取 `_orchestrate_single_std_code(task, std_code) -> OrchestrateSingleResult`  
- [x] 实现 `ensure_step4_indicators(task_id, compare_pairs, request)`  
- [x] 从 `compare_pairs` 展开去重标准号（**禁止**使用 `list_latest_side_std_codes_for_task` 作为本接口输入）  
- [x] 写 `indicator_bundle_json`、可选 `dify_run_metadata_json`  
- [x] 计算 `all_ready = len(missing_gb_files) == 0`

### 6.3 路由（`backend/apps/compliance/api/router.py`）

- [x] `@router.post("/evaluations/{task_id}/step/4/indicators/ensure", response=Step4IndicatorsEnsureOut)`

### 6.4 测试建议

- [x] `compare_pairs` 两列去重： publication + latest 共 3 个唯一号  
- [x] 已有指标的标准 → `status=ready`，不触发 Dify②  
- [x] 有 `std_file_path` 无指标 → 触发 Dify②，`parsed_via_dify2`  
- [x] 无路径 → `missing_gb_files`，`all_ready=false`  
- [x] 上传国标后再次 ensure → `all_ready=true`  
- [x] `current_step=3` → 422  
...
**后端自检（MySQL 真库）**：在 `backend` 目录、已配置 `MYSQL_*` 且已导入 v1.0-sql 时执行：

```bash
python scripts/verify_ensure_mysql.py
```

预期输出 `RESULT: PASS`（缺件 → 上传国标 → 再次 ensure → `all_ready=true`）。
...
| 2026-05-24 | 后端实现 POST ensure；路由/Schema/服务层/单元与 MySQL 集成测试完成；MySQL 联调验证通过 |
