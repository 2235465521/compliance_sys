...
| `inferred_historical_std_code` | `string \| null` | **可选**：无年号引用经谱系前缀族补全后的锚点标准号；新写入子项若解析结果含此字段会带上。GET 时即使缺省，只要存在带年号的 **`full_std_at_publication`** 且 `resolution_path` 为 **`historical_then_pedigree`**，后端仍会判定可比对。 |
| `full_std_name_at_publication` | `string \| null` | **展示用**：与 `full_std_at_publication` 相同的标准号在 **`national_standard_basic`** 中命中时的 **`std_name`**；无完整号、主表无行或名称为空时为 `null`。**不参与**合规自动判定。 |
| `latest_std_primary` | `string` | 谱系侧主展示现行号；无法给出时为 **空字符串** `""`（**不会**再用引用原文冒充现行）。 |
| `latest_std_name` | `string \| null` | **展示用**：与 `latest_std_primary`（非空时）在主表命中时的标准名称；否则 `null`。 |
...
2. **标准名称**：`full_std_name_at_publication` / `latest_std_name` 仅按标准号在 **`national_standard_basic`** 中 **只读补全展示**，与合规结论无关；主表未导入或 `std_code` 不一致时为 `null`。  
3. **合规自动结论**：**仅认谱系表给出的现行信息**；无谱系命中或谱系无有效 `latest_std_code` 时，**现行侧为空**，且 **`compliance_assessable` 为 `false`**。  
4. **整文件 `compliant`**：要求 **每一条**引用 **`compliance_assessable === true`** 且 **`citation_matches_latest === true`**。因此混有 **非 GB**、**QB 轻工行业标准**（不参与国标自动查新）、**缺 Q/ 企标号**、**谱系未命中** 等任一情况时，整文件结论一般为 **`undetermined`**，而不是「部分合规」。
...
- **不必**在 PATCH 体中提交 `full_std_name_at_publication` / `latest_std_name`：GET/PATCH 的响应序列化时会按当前标准号 **重新查询主表** 填充（落库 JSON 可不存名称）。  
- 服务端保存前会对每条调用 **`enrich_normative_reference_row`**：  
  - 会先根据 **`resolution_path`**、**`referenced_std_code`/`query_bz_id`**、**`full_std_*`/`latest_std_*`**（含 legacy 键）**重算** **`compliance_assessable`**，避免历史上 **`compliance_assessable` 缺省被当成 false** 导致整文件长期 **`undetermined`**。  
  - 若 **`compliance_assessable`** 为 `false`（推断结果），会把 **`citation_matches_latest`** 置为 **`null`**。  
...
- [ ] 「时点完整号」绑定 **`full_std_at_publication`**；若有 **`full_std_name_at_publication`** 可副标题或 tooltip 展示。  
- [ ] 「现行号」旁可展示 **`latest_std_name`**（与 **`latest_std_primary`** 同列或第二行）。  
- [ ] 说明文案绑定 **`explanation`**。  
...
| 单条精简输出 | 同上 → `normative_reference_row_out`、`normative_reference_row_core`、`attach_normative_reference_std_names`、`enrich_normative_reference_row` |
| 主表名称查询 | `backend/apps/standards/services/reference_resolution.py` → `fetch_national_standard_names_by_codes` |
...
| 2026-05-11 | 增补：`full_std_name_at_publication`、`latest_std_name`（`national_standard_basic` 展示用）。 |
| 2026-05-11 | GET/PATCH：`enrich_normative_reference_row` 会重算 `compliance_assessable`（修复缺省键被误判为不可比对）。 |
| 2026-05-11 | `historical_then_pedigree`：精简行缺 `inferred_historical_std_code` 时，用带年号的 `full_std_at_publication` 等价推断可比对性；`normative_reference_row_core` 可选透出 `inferred_historical_std_code`。 |
| 2026-05-23 | **`qb_enterprise_citation`** 说明更正：引用号以 **`QB`** 开头表示**轻工行业标准**（非企标 `Q/…`）；`explanation` 文案已同步。 |
